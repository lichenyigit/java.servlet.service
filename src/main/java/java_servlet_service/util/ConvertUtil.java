...
import java_servlet_service.exception.ConvertFailedException;
...
import java_servlet_service.exception.ParameterConvertException;
...
public class ConvertUtil {
	...
	/**
	 * <h1>从request对象中获取Long</h1>
	 * <p>从request对象中获取指定参数，并以十进制的形式转换为Long。</p>
	 * <p>如果对应的参数不存在，或转换失败，返回默认值</p>
	 * @author lichenyi
	 * @date 2017-7-18 13:01
	 */
	public static Long getLongFromRequestParam(HttpServletRequest request,String paramName, Long defaultVal) throws ParameterConvertException {
		String val = request.getParameter(paramName);
		if(val==null){
			return defaultVal;
		}
	...
	public static AuditStatusEnum getAuditStatus(HttpServletRequest request, String paramName){
		String value = request.getParameter(paramName);
		if(StringUtil.isBlank(value))
			return null;
		return AuditStatusEnum.getAuditStatus(value);
	}

	public static EnableEnum getEnableEnum(HttpServletRequest request, String paramName) throws ParameterConvertException {
		if(request.getParameter(paramName) == null)
			return null;
		int code = getIntFromRequestParam(request, paramName);
		return EnableEnum.getEnableEnum(code);
	}

	public static OrderByTypeEnum getOrderByTypeEnum(HttpServletRequest request, String paramName) throws ParameterConvertException {
		if(request.getParameter(paramName) == null)
			return null;
		int code = getIntFromRequestParam(request, paramName);
		return OrderByTypeEnum.getOrderByTypeEnum(code);
	}
